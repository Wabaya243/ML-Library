...
# Architecture attention causale standard (pas de sliding window
# interleavée) -> bien supportée par sdpa, contrairement à Gemma-3
...
# os.environ["HF_TOKEN"] = "TON_TOKEN_ICI"  # nécessaire si modèle gated
...
# Qwen2.5 utilise le format ChatML standard:
# <|im_start|>system\n...<|im_end|>\n<|im_start|>user\n...<|im_end|>\n
# <|im_start|>assistant\n...<|im_end|>
@dataclass
class DataCollatorAssistantOnly:
    """
    Masque tout sauf la réponse assistant.
    Auto-détecte le vrai marker depuis un exemple réel (apply_chat_template)
    au lieu de le deviner -- évite les bugs de format vus avec Phi-4/Gemma.
    Marker encodé EN CONTEXTE pour éviter la fusion de sous-mots.
    """
    tokenizer: object
    ...
    def __post_init__(self):
        ...
        print(f"[Collator Qwen] Marker détecté: {marker_found!r}")
        ...
        print(f"[Collator Qwen] Marker IDs: {marker_ids} -> "
...
#  Sauvegarde 
adapter_out = "./qwen2.5-7b-unikin-lora"
...
enc = build_inputs("Je suis en L1, j'hésite entre Informatique et Géo. J'aime le language Python, je suis niveau maths moyen comment faire ?.")
with torch.no_grad():
    output = mdl.generate(
        **enc,
        max_new_tokens=220,
