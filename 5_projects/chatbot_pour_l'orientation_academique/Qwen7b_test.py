...
# Test d’un modèle LoRA fine-tuné sur Qwen2.5-7B-Instruct
...
ADAPTER_PATH = "./qwen2.5-7b-unikin-lora"
...
# 2. Tokenizer (Chargement du format ChatML de Qwen)
...
# Extraction stricte de l'ID du jeton de fin de tour ChatML de Qwen (<|im_end|>)
try:
...
# Implémentation SDPA (parfaitement supportée par Qwen2.5, gain VRAM majeur)
mdl.config.attn_implementation = "sdpa"

# Chargement des poids de votre adaptateur LoRA
...
def ask(question: str, max_new_tokens=450):
    ...
    # Génération automatique de la structure ChatML (<|im_start|>...)
    ...
    with torch.no_grad():
        output = mdl.generate(
            **enc,
            ...
            temperature=0.7,
            top_p=0.90,
            repetition_penalty=1.12,  # Bloque les répétitions de phrases en français
            eos_token_id=IM_END_TOKEN_ID, # Force Qwen à s'arrêter proprement sans halluciner l'utilisateur
...
with open("Data/history_qwen.json", "w", encoding="utf-8") as f:
    json.dump(history, f, ensure_ascii=False, indent=2)

print("\n Historique Qwen2.5 sauvegardé avec succès.")