...
# LoRA 4-bit — Llama-3.1-8B-Instruct (UNIKIN)
...
os.environ["HF_TOKEN"] = "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"  # modèle gated, accès + token requis
...
# Auto-détecte le marker depuis un exemple réel (apply_chat_template) au lieu
# de le deviner, et encode le marker EN CONTEXTE (pas isolément) pour éviter
# tout risque de désalignement de tokenisation -- même méthode validée que
# pour Mistral / Phi-4-mini / Qwen2.5.
@dataclass
class DataCollatorAssistantOnly:
    ...
    def __post_init__(self):
        ...
        print(f"[Collator Llama] Marker détecté: {marker_found!r}")
        ...
        print(f"[Collator Llama] Marker IDs: {marker_ids} -> "
...
# Llama 3 n'a pas de pad_token par défaut -> on en ajoute un dédié
...
def to_chat(example):
    ...
    encoded = tokenizer(text, truncation=True, max_length=740, add_special_tokens=False)
...
# Resize embeddings car on a ajouté <|pad|> au tokenizer
...
#  LoRA 
# Llama 3 MLP: gate_proj / up_proj / down_proj (identique à Qwen/Mistral)
lora = LoraConfig(
...
sample_batch = [
    tokenizer(t, truncation=True, max_length=540, add_special_tokens=False)
...
#  Sauvegarde
adapter_out = "./llama3.1-8b-unikin-lora"
