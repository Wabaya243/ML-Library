...
# LoRA 4-bit — Falcon3-7B-Instruct (TII) — UNIKIN
# RTX 4070 Laptop 8GB — rapide et stable
#
# Architecture moderne: GQA (12 query heads / 4 kv heads), SwiGLU,
# RMSNorm -- même famille que Llama/Mistral/Qwen, PAS de sliding
# window interleavée. target_modules standards attendus.
# Supporte nativement le français. Pas gated, Apache 2.0 / TII Falcon License.
...
# Falcon3-Instruct utilise un chat template type ChatML standard via
# apply_chat_template (rôles system/user/assistant). On auto-détecte le
...
class DataCollatorAssistantOnly:
    ...
    def __post_init__(self):
        ...
        print(f"[Collator Falcon3] Marker détecté: {marker_found!r}")
        ...
        print(f"[Collator Falcon3] Marker IDs: {marker_ids} -> "
...
#  Modèle de base 
model_name = "tiiuae/Falcon3-7B-Instruct"  # PAS "tiiuae/falcon-7b" (archi différente)
...
def to_chat(example):
    ...
    encoded = tokenizer(text, truncation=True, max_length=700, add_special_tokens=False)
...
#  LoRA 
# Falcon3 (GQA + SwiGLU) -> mêmes target_modules que Llama/Mistral/Qwen.
# Si get_peft_model affiche 0 trainable params, vérifier les vrais noms
# de module avec: [n for n, _ in model.named_modules() if "proj" in n][:20]
lora = LoraConfig(
...
#  Sauvegarde 
adapter_out = "./falcon3-7b-unikin-lora"
