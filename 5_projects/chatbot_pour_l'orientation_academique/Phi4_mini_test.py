# Test d’un modèle LoRA fine-tuné sur Phi-4-mini-Instruct
...
ADAPTER_PATH = "./phi4-mini-unikin-lora"
...
# Alignement avec vos choix d'entraînement
...
# Optionnel mais recommandé : forcer l'implémentation SDPA pour la rapidité
...
# 4. Gestion de conversation (chat-template ChatML / Phi-4)
...
def ask(question: str, max_new_tokens=450):
    ...
    # Utilisation du template natif de Phi-4-mini
    ...
    # Détection dynamique de l'ID du token d'arrêt <|end|> mentionné dans vos notes
    ...
    with torch.no_grad():
        output = mdl.generate(
            **enc,
            ...
            do_sample=True,          # Activé pour plus de fluidité sur les réponses longues
            temperature=0.7,
            top_p=0.90,
    ...
    # Nettoyage additionnel au cas où le token brut s'affiche en texte
    if "<|end|>" in resp:
        resp = resp.split("<|end|>")[0].strip()
...
with open("Data/history_phi4.json", "w", encoding="utf-8") as f:
    json.dump(history, f, ensure_ascii=False, indent=2)

print("\n Historique Phi-4-mini sauvegardé avec succès.")