...
# ==========================================================
# LoRA 4-bit — Qwen2.5-7B-Instruct (UNIKIN)
...
class DataCollatorAssistantOnly:
    ...
    def __call__(self, features):
        ...
        # encode patterns une fois
        start_ids = torch.tensor(
            self.tokenizer.encode(self.assistant_header, add_special_tokens=False),
            dtype=torch.long
        )
        end_ids = torch.tensor(
            self.tokenizer.encode(self.end_token, add_special_tokens=False),
            dtype=torch.long
        )

        # recherche vectorisée des motifs
        B, L = input_ids.size()

        # fonction pour trouver un motif dans chaque séquence
        def find_pattern(pattern):
            pat_len = pattern.size(0)
            expanded = input_ids.unfold(1, pat_len, 1)
            matches = (expanded == pattern).all(dim=2)
            return matches

        # positions start/end
        start_matches = find_pattern(start_ids)
        end_matches   = find_pattern(end_ids)

        for b in range(B):
            starts = torch.where(start_matches[b])[0]
            if len(starts) == 0:
                continue

            # on prend le DERNIER <|im_start|>assistant
            s = starts[-1] + start_ids.size(0)

            # chercher la première fin après start
            ends_after = torch.where(end_matches[b] & (torch.arange(L- end_ids.size(0) +1) >= s))[0]
            e = ends_after[0] if len(ends_after) > 0 else L
...
class PerfAndMem(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        # vider le cache toutes les 100 itérations seulement
...
tokenizer.truncation_side = "right"   # on coupe la fin si jamais, pas le début
...
# ========================= Trainer TRL (SFT) =========================
cfg_kwargs = dict(
    output_dir="./qwen2-7b-unikin",
    per_device_train_batch_size=1,      # 8 Go VRAM -> 1
    ...
    logging_steps=100,
    num_train_epochs=4,                 # 3-4 si dataset pas énorme
    save_steps=300,
    save_total_limit=2,
...
gc.collect()
print(f"VRAM dispo juste avant train : {torch.cuda.mem_get_info()[0]/1e9:.2f} Go libres")

trainer.train(resume_from_checkpoint=True)


# ========================= Sauvegarde adapter LoRA + tokenizer =========================
adapter_out = "./qwen2-7b-unikin-lora"
