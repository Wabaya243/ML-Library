...
class PerfAndMem(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        # vider le cache toutes les 50 itérations seulement
...
# ========================= Modèle de base =========================
# a tester DeepSeek-R1-0528-Qwen3-8B
...
def to_chat(example):
    ...
    assistant = truncate_text(strip_end_tag(resp_raw), 2000)
...
cfg_kwargs = dict(
    output_dir="./deepseek-qwen7b-unikin",
    ...
    logging_steps=100,
    num_train_epochs=5,              # 3-4 si dataset pas énorme
    save_steps=300,
    save_total_limit=3,
...
adapter_out = "./deepseek-qwen7b-unikin-lora-final"
trainer.model.save_pretrained(adapter_out)
tokenizer.save_pretrained(adapter_out)


# ========================= PHASE DE TEST =========================
path = r"C:/Users/OMEN/Documents/Cours I.A (Machine Learning)/5. Projet/chatbot_pour_l'orientation_academique/"
...
enc = build_inputs("bonjour bot ça va ?.")
with torch.no_grad():
    output = mdl.generate(
    **enc,
    max_new_tokens=220,
    ...
    eos_token_id=tok.eos_token_id,  # DeepSeek : </s>
    streamer=streamer
)
