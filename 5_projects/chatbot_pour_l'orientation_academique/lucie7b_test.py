# Test d’un modèle LoRA fine-tuné sur Lucie-7B-Instruct
...
ADAPTER_PATH = "./lucie-7b-unikin-lora"

# Quantization 4-bit optimisée pour la VRAM de 8Go
...
# Alignement strict avec votre script d'entraînement Lucie
...
# Extraction de l'ID du jeton de fin de tour de parole (End Of Turn) de Lucie/Llama3
try:
    EOT_TOKEN_ID = tok.convert_tokens_to_ids("<|eot_id|>")
    print("EOT token trouvé (<|eot_id|>) -> id:", EOT_TOKEN_ID)
except Exception:
    EOT_TOKEN_ID = tok.eos_token_id
    print("Attention: <|eot_id|> non trouvé, repli sur eos_token_id:", EOT_TOKEN_ID)
...
# Forcer l'implémentation SDPA pour l'économie de VRAM et la vitesse
mdl.config.attn_implementation = "sdpa"

# Chargement de l'adaptateur LoRA entraîné
mdl = PeftModel.from_pretrained(mdl, ADAPTER_PATH)
mdl.eval()

print("Adaptateurs actifs :", mdl.active_adapters)

# 4. Gestion de la conversation (Format Llama 3 / Lucie)
...
def ask(question: str, max_new_tokens=450):
    ...
    # Génération du prompt selon le chat_template de Lucie
    ...
    with torch.no_grad():
        output = mdl.generate(
            **enc,
            ...
            temperature=0.7,
            top_p=0.90,
            repetition_penalty=1.12,  # Aide à éviter les répétitions en français
            eos_token_id=EOT_TOKEN_ID, # Utilisation de l'EOT de Lucie pour couper la génération au bon endroit
...
with open("Data/history_lucie.json", "w", encoding="utf-8") as f:
    json.dump(history, f, ensure_ascii=False, indent=2)

print("\n Historique Lucie sauvegardé avec succès.")