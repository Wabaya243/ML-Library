...
# LoRA 4-bit — Phi-4-mini-Instruct (UNIKIN)
# RTX 4070 Laptop 8GB — rapide et stable

'''
Le format se termine par <|end|> puis <|endoftext|>. Donc pour l'inférence plus tard dans main.py, 
le vrai token de fin de tour est <|end|>, pas <|im_end|> (qui n'existe probablement pas dans ce tokenizer). 
On corrigera eos_token_id dans le service d'inférence une fois l'entraînement validé — note pour plus tard.
'''
...
# Phi-4-mini utilise le format ChatML : <|im_start|> / <|im_end|>
# Identique à Qwen — seul le tokenizer change
import torch
...
class DataCollatorAssistantOnly:
    """
    Masque tout sauf la réponse assistant (format ChatML Phi-4-mini).
    Compatible SFTTrainer (features déjà tokenisés en input_ids/attention_mask).

    Au lieu de deviner le séparateur exact (<|im_sep|> vs \n selon variante
    Phi), on détecte automatiquement le vrai marker en inspectant un
    exemple réel du dataset au moment de l'initialisation.
    """
    tokenizer: object
    sample_text_for_detection: str = None  # un texte réel produit par apply_chat_template

    def __post_init__(self):
        if self.sample_text_for_detection is None:
            raise ValueError(
                "Passe un texte réel (déjà passé par apply_chat_template) "
                "via sample_text_for_detection pour auto-détecter le marker."
            )

        text = self.sample_text_for_detection

        # Trouve où commence le tour assistant en cherchant le pattern
        # générique : un bloc qui contient "assistant" suivi de séparateurs
        # variables, juste après la fin du tour user.
        # On cherche la DERNIÈRE occurrence du mot "assistant" précédée
        # d'un marker de début de tour connu pour ChatML/Phi.
        # Remplace l'ancienne liste 'candidates' par celle-ci :
        candidates = [
            "<|assistant|>",
            "<|im_start|>assistant",
        ]
        ...
        if marker_found is None:
            raise ValueError(
                f"Aucun marker assistant connu trouvé dans le texte d'exemple. "
                f"Texte (300 derniers chars): ...{text[-300:]}"
            )

        self.assistant_marker = marker_found
        print(f"[DataCollatorAssistantOnly] Marker détecté: {marker_found!r}")

        # Encode le marker EN CONTEXTE (collé après le tour user réel),
        # pas isolément, pour éviter le bug de fusion de sous-mots.
        marker_pos_in_text = text.rfind(marker_found)
        # On prend ~20 caractères avant le marker comme contexte gauche
        context_start = max(0, marker_pos_in_text - 20)
        probe_text = text[context_start:marker_pos_in_text] + marker_found

        probe_ids = self.tokenizer.encode(probe_text, add_special_tokens=False)
        context_ids = self.tokenizer.encode(
            text[context_start:marker_pos_in_text], add_special_tokens=False
        )

        # Le marker = la fin de probe_ids après le préfixe de contexte
        marker_ids = probe_ids[len(context_ids):]
        if len(marker_ids) == 0:
            raise ValueError("Échec extraction des IDs du marker assistant.")

        self._marker_ids = torch.tensor(marker_ids, dtype=torch.long)
        print(f"[DataCollatorAssistantOnly] Marker IDs: {marker_ids} "
              f"-> decode: {self.tokenizer.decode(marker_ids)!r}")

    def _find_last_match(self, seq: torch.Tensor, pattern: torch.Tensor):
...
# Phi-4-mini a <|endoftext|> comme eos — on l'utilise comme pad
...
def to_chat(example):
    ...
    #  Template natif Phi-4-mini (ChatML) via le tokenizer HF
    # Pas besoin de format_phi_chat() manuel
...
#  LoRA 
# Phi-4-mini : même architecture MLP que Llama/Qwen
lora = LoraConfig(
...
#  Sauvegarde 
adapter_out = "./phi4-mini-unikin-lora"
