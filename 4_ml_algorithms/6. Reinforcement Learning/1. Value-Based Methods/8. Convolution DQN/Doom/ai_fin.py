...
import experience_replay, image_preprocessing, env
...
class SoftmaxBody(nn.Module):
    ...
    def forward(self, outputs):
        probs = self.softmax(outputs / self.T)
...
class AI:
    ...
    def __call__(self, inputs):
        ...
        inputs = Variable(torch.from_numpy(inputs)).to(Device)
        # on passe les inputs dans le brain
        outputs = self.brain(inputs)
        # on passe les outputs dans le body
...
doom_env = env.VizdoomEnv("deadly_corridor.cfg", difficulty=4)
...
target_cnn = CNN(number_actions)
target_cnn.load_state_dict(cnn.state_dict())
target_cnn.to(Device)


# Corps de l'agent : transforme les sorties du réseau en probabilités (Softmax)
# T = température : plus T est élevé, plus l'agent explore
softmax_body = SoftmaxBody(T=3.0)

# Création de l'agent IA avec un cerveau (CNN) et un corps (Softmax)
agent = AI(brain=cnn, body=softmax_body)
...
memory = experience_replay.ReplayMemory(
    n_steps=n_steps,
    capacity=100000 # recomender pour doom 20000
)
...
def eligibilityTrace(batch):
    ...
    for series in tqdm(batch, desc="Elig. Trace Fast"):
        ...
        G = 0.0 if done else q_target[-1].max().item()
        ...
        for t in reversed(range(T)):
            ...
            G = rewards[t] + gamma * ((1 - lam) * q_target[t].max().item() + lam * G)
...
# hyperparams
loss = nn.MSELoss()
optimizer = optim.Adam(cnn.parameters(), lr=0.00025)
nb_epoch = 500
...
for epoch in range(nb_epoch):
    ...
    for i, batch in enumerate(tqdm(memory.sample_batch(128), desc=f"Epoch : {epoch}/{nb_epoch}")):
    ...
    softmax_body.T = max(0.5, softmax_body.T * 0.995)
    
    print(f"Epoch {epoch} | Temperature = {softmax_body.T:.3f}")
