...
class MyNormalizedEnv(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        ...
        return ret  # NE PAS faire np.expand_dims ici
...
class PreprocessImage(ObservationWrapper):
    ...
    def __init__(self, env, height=84, width=84, grayscale=True, crop=lambda img: img):
        ...
        n_colors = 1 if grayscale else 3

        # Nouvel espace d'observation
        self.observation_space = Box(0, 255, [n_colors, height, width], dtype=np.uint8)
    ...
    def observation(self, img):
        ...
        # Recadrage
        img = self.crop(img)
        # Forcer la conversion en float32
...
def create_atari_env(env_id, video=False, seed=None):
    ...
    env = PreprocessImage(env, height=84, width=84, grayscale=True)
    env = MyNormalizedEnv(env)
    obs, _ = env.reset()
    print(obs.shape)  # doit afficher (1,84,84)
