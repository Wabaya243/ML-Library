...
class VizdoomEnv(gym.Env): 
    ...
    def step(self, action):
        ...
        if local_action[self.attack_local_idx] == 1:
            if ammo > 0:
                reward -= 0.01  # petit malus pour le tir
            else:
                reward -= 0.3   # gros malus pour tirer sans munitions
        ...
        reward += 6.0 * (killcount - self.last_killcount)
        
        
        # petit malus par step (pression temporelle)
        reward -= 0.1
