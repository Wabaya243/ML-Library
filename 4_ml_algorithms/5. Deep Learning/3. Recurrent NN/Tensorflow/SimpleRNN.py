...
from tensorflow.keras.layers import SimpleRNN         # Import de la couche LSTM (Long Short-Term Memory), adaptée au traitement de séquences textuelles.
...
model.add(Bidirectional(SimpleRNN(256, dropout=0.3, recurrent_dropout=0.3)))  
# SimpleRNN bidirectionnel avec 256 neurones, dropout pour éviter l’overfitting
...
history = model.fit(
    ...
    epochs=2,                               # Nombre d’itérations d’apprentissage
    batch_size=256                          # Nombre d’exemples traités avant mise à jour des poids
)
...
# Exemple de phrase que l'utilisateur écrit
new_text = "I hate you, you are so dumb!"

# 1. Prétraitement du texte
clean_text = preprocess_text(new_text)  # utilise ta fonction déjà définie

# 2. Conversion en séquence d'indices
seq = token.texts_to_sequences([clean_text])

# 3. Appliquer le padding
